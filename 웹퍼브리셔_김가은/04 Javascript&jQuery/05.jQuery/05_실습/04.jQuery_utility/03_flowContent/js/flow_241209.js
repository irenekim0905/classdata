...
$(document).ready(function () {
    ...
    //자동실행!
    autocall = setInterval(flow, 30); //흘러가는 속도 조절 = 자동실행 시간으로 조절!
    ...
    var boxWidth2 = $('.flow li').first().outerWidth(); //총 너비
    console.log(boxWidth2); //384 */
...
function flow() {
    ...
    var boxWidth2 = $('.flow li').first().outerWidth();

    // if else문
    // 이동한 픽셀수(moveNum)가 li 하나의 너비보다 커졌을 때 -> 다음을 위한 준비!
    if (moveNum > boxWidth2) {
        //(왼쪽으로 흘러가서)사라진 첫번째 li를 .flow의 맨 뒤로 이동!
